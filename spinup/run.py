...
from spinup.algos.pytorch.tabsarsa.core import id_update, decrease_by_reward, decrease_by_lr_times_reward, \
    set_to_last_return, increase_by_lr_times_return, increase_by_lrsq_times_return, set_to_lr_times_return,\
    increase_by_q_dif, set_to_lr_times_return_div_length, id_update_start
...
BASE_ALGO_NAMES = ['vpg',
                   ...
                   'exploit',
                   'tabsarsa',
                   'tabsarsa_store_q_tables',
                   'store_q_table',
                   'store_vi_q_table',
                   'test_xis_for_q_star',
                   'trpo', 'ppo', 'ddpg', 'td3', 'dqn']
...
def parse_and_execute_grid_search(cmd, args):
    ...
    if "seed" in arg_dict:
        if len(arg_dict["seed"]) == 1 and "to" in arg_dict["seed"][0]:
            first, last = arg_dict["seed"][0].split("to")
            arg_dict["seed"] = list(range(int(first), int(last)))
...
if __name__ == '__main__':
    ...
    if cmd in valid_help:
        ...
        args = sys.argv[2:]
        if "--exp_name" in args:
            exp_name = args[args.index("--exp_name") + 1]
            if "--add_date" in args:
                exp_name = datetime.now().strftime('%Y%m%d_%H%M%S')[2:] + "_" + exp_name
                args[args.index("--exp_name") + 1] = exp_name
                args.remove("--add_date")

            if "--comment" in args:
                # comment_kw_index = args.index("--comment")
                comment = args[args.index("--comment") + 1]
                dir_name = os.path.join("comments", exp_name)
                os.makedirs(dir_name, exist_ok=True)
                file_name = os.path.join(dir_name, "comment.txt")
                f = open(file_name, "a")
                f.write(comment)
                f.close()
                args.remove("--comment")
        ...
        plot_results = "--plot_results" in args
        if plot_results:
            args.remove("--plot_results")

        plot_tradeoffs = "--plot_tradeoffs" in args
        if plot_tradeoffs:
            args.remove("--plot_tradeoffs")

        plot_lc = "--plot_lc" in args
        if plot_lc:
            args.remove("--plot_lc")
        ...
        # Plot after all experiments have been executed.

        if plot_lc and "--exp_name" in args:
            args = [sys.executable if sys.executable else 'python',
        ...
        if plot_results and "--exp_name" in args:
            args = [sys.executable if sys.executable else 'python',
                    'spinup/utils/plot.py',
                    f"data/{exp_name}",
                    "--use_nice_names",
                    "--x", "TotalActionsEvaluated"]
                    # "--x", "xi"]
            subprocess.check_call(args, env=os.environ)

        if plot_tradeoffs and "--exp_name" in args:
            args = [sys.executable if sys.executable else 'python',
                    'spinup/utils/plot_tradeoffs.py',
                    f"data/{exp_name}",
                    "--use_nice_names"]
            subprocess.check_call(args, env=os.environ)
